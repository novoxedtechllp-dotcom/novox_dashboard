      ...
      .map((item) => {
        if (item.id === "leave") {
          return {
            ...item,
            label:
              userRole === "STUDENT" ? "Leave Requests" : "Leave Management",
          };
        }
    ...
    <>
      ...
      >
        ...
        <div className="w-[220px] min-w-[220px] flex flex-col h-full">
            ...
            <div className="flex justify-between items-center">
              <div className="flex flex-col items-start px-2">
                <img
                  ...
                  alt="Novox Edtech"
                  className="h-[44px] w-[180px] object-contain object-left -ml-2"
                />
          ...
          {/* Main Nav Container */}
          <nav className="flex-1 flex flex-col gap-1.5 overflow-y-auto mt-4 pb-4 pr-2 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
            {visibleNavItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <Icon
                    size={18}
                    className={`transition-transform duration-300 group-hover:scale-110 ${isActive ? "text-[#003F87]" : "text-slate-400 group-hover:text-slate-600"}`}
                  />
                  <span className="text-[14px] leading-none whitespace-nowrap mt-0.5">
                    {item.id === "leave" && (userRole === "ADMIN" || isHR)
                      ? "Leave Management"
                      : item.label}
                  </span>
                </Link>
          ...
          <div className="mt-auto pt-4 border-t border-slate-100 shrink-0 pr-3 flex flex-col gap-1.5">
            {userRole === "ADMIN" && (
              ...
              >
                ...
                <Settings
                  size={18}
                  className={`transition-transform duration-300 group-hover:scale-110 ${activeTab === "settings" ? "text-[#003F87]" : "text-slate-400 group-hover:text-slate-600"}`}
                />
                <span className="text-[14px] leading-none whitespace-nowrap mt-0.5">
                  Settings
                </span>
            ...
            {onLogout && (
              ...
              >
                <LogOut
                  size={18}
                  className="text-[#D80000]/80 group-hover:text-[#D80000] transition-transform duration-300 group-hover:-translate-x-1"
                />
                <span className="text-[14px] leading-none whitespace-nowrap mt-0.5">
                  Log Out
                </span>
