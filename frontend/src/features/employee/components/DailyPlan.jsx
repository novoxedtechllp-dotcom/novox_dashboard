...
const DailyPlan = ({ userType, userId }) => {
  ...
  const handlePrevDay = () => {
    ...
    setTimeout(() => {
      ...
      setSelectedDate(buildDateString(d));
      setSlideDirection("");
    }, 180);
  };
    ...
    if (userType === "ADMIN") {
      const fetchEmployees = async () => {
        try {
          ...
          emps = emps.filter(
            (emp) =>
              emp.course_instructors && emp.course_instructors.length > 0,
          );
  ...
  const handleStudentReview = async (sessionId, reviewText, rating) => {
    try {
      ...
      const response = await fetch(
        `/api/v1/students/${userId}/mentoring-sessions/${sessionId}/review`,
        {
          method: "PATCH",
          headers,
          body: JSON.stringify({
            student_review: reviewText,
  ...
  return (
    <div className="flex flex-col gap-6 p-6 md:p-8 h-full bg-[#FAFBFC] relative overflow-hidden">
      {toast && (
        <div
          className={`fixed bottom-8 right-8 z-[9999] px-6 py-4 rounded-xl shadow-2xl font-bold text-sm transform transition-all duration-300 flex items-center gap-3 ${toast.type === "error" ? "bg-red-500 text-white" : "bg-slate-800 text-white"}`}
        >
          {toast.type === "error" ? (
            <X size={18} />
          ) : (
      ...
      {/* Header */}
<div className="bg-white rounded-2xl xl:rounded-3xl border border-slate-200 shadow-sm p-4 sm:p-5 md:p-6 lg:p-7">
  <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
    ...
    <div className="text-center xl:text-left">
      ...
      <p className="mt-2 flex items-center justify-center xl:justify-start gap-2 text-sm sm:text-base text-slate-500">
        <CalendarDays size={16} />
        View and log mentoring sessions.
      </p>
    </div>
    ...
    <div className="w-full xl:w-auto flex flex-col gap-4">
      ...
      <div className="flex flex-wrap justify-center xl:justify-end gap-3">
        ...
        {/* Month */}
        <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm w-full sm:w-auto">
          <span className="text-[10px] uppercase font-black tracking-[0.18em] text-slate-400 whitespace-nowrap">
            Month
          </span>
                ...
                className={`
                  ...
                  ${
                    d.dateString === selectedDate
                      ? "bg-[#003F87] text-white shadow-lg scale-105 border border-blue-600"
                      : "bg-white border border-slate-200 hover:border-[#003F87] hover:-translate-y-1 hover:shadow-lg text-slate-600"
                  }
                `}
              >
                <span
                ...
                <span className="text-lg sm:text-xl font-bold leading-none mt-1">
                  {d.dayNumber}
                </span>
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                ...
                <div className="relative">
                  ...
                  >
                    <span>
                      {employeesList.find(
                        (emp) => emp.id === selectedAdminEmployeeId,
                      )
                    ...
                    <ChevronDown
                      size={16}
                      className={`text-slate-400 transition-transform ${isStaffDropdownOpen ? "rotate-180" : ""}`}
                    />
                  </div>
                  {isStaffDropdownOpen && (
                    <div className="absolute z-50 top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-xl shadow-lg max-h-[300px] flex flex-col overflow-hidden">
                      ...
                      <div className="overflow-y-auto flex-1">
                        ...
                        {employeesList.filter((emp) =>
                          ...
                          <div className="px-4 py-6 text-sm text-slate-500 text-center italic">
                            No mentors found
                          </div>
                        )}
                      </div>
                ...
                <div className="relative z-10 flex flex-col items-center text-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                      Record a mentoring session with a student.
                    </p>
                  </div>
                  <button
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-slate-50/50">
            {loading ? (
              <div className="h-full flex items-center justify-center">
                <LoadingSpinner text="Fetching sessions..." />
              </div>
            ) : sessions.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center py-10 text-center max-w-sm mx-auto">
                ...
                <p className="text-sm text-slate-500 leading-relaxed mb-6">
                  No mentoring sessions logged for this date.
                </p>
                {userType === "EMPLOYEE" && (
                  <button
                    onClick={() => setIsLogSessionModalOpen(true)}
                    className="px-6 py-2.5 bg-white border-2 border-slate-200 text-slate-700 font-bold rounded-xl hover:border-[#003F87] hover:text-[#003F87] transition-colors"
                  >
                    Log Session Now
                  </button>
                )}
                ...
                {sessions.map((session, index) => (
                  <div key={session.id} className="relative pl-6 md:pl-10">
                    {index !== sessions.length - 1 && (
                      <div className="absolute left-3 md:left-[19px] top-8 bottom-[-24px] w-[2px] bg-slate-200"></div>
                    )}
                      ...
                      <div className="p-5 border-b border-slate-100 flex flex-col gap-4 bg-white relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="flex items-center gap-2 mb-2">
                              <span className="bg-[#E5F0FF] text-[#003F87] text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider">
                                {
                                  session.course_submodules?.course_modules
                                    ?.courses?.name
                                }
                              </span>
                          ...
                          <div className="px-3 py-1 bg-slate-100 text-slate-600 font-bold text-xs rounded-lg flex items-center gap-1.5">
                            <Clock size={12} /> {session.time_taken_minutes}{" "}
                            mins
                          </div>
                        </div>
                        ...
                        {userType !== "STUDENT" && session.students && (
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-sm font-semibold text-slate-700">
                              Student: {session.students.first_name}{" "}
                          ...
                          session.employee_profiles && (
                            <div className="flex items-center gap-2 mt-1">
                              <div className="w-6 h-6 rounded-full bg-purple-100 flex items-center justify-center text-purple-700 font-bold text-[10px]">
                                {session.employee_profiles.first_name?.[0]}
                                {session.employee_profiles.last_name?.[0]}
                              </div>
                              <span className="text-sm font-semibold text-slate-700">
                                Mentor: {session.employee_profiles.first_name}{" "}
                                {session.employee_profiles.last_name}
                              </span>
                            </div>
                          )}
                      </div>
                          ...
                          {session.student_review || session.student_rating ? (
                            ...
                            <StudentReviewForm
                              ...
                              onSubmit={(review, rating) =>
                                handleStudentReview(session.id, review, rating)
                              }
                            />
                          ) : (
...
const StudentReviewForm = ({ sessionId, onSubmit }) => {
  ...
  return (
    <form
      ...
      className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm flex flex-col gap-3"
    >
...
const LogSessionModal = ({
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const stuRes = await fetch(
          `/api/v1/students?instructorId=${userId}&limit=1000`,
          { headers },
        );
        ...
        const topRes = await fetch(
          `/api/v1/employees/${userId}/available-topics?date=${selectedDate}`,
          { headers },
        );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedStudent || !selectedTopic || !timeTaken || !note.trim())
      return;
    setSubmitting(true);
    try {
      const headers = getAuthHeaders();
      const res = await fetch(
        `/api/v1/employees/${userId}/mentoring-sessions`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6 md:px-8 border-b border-slate-100 flex justify-between items-center bg-white shrink-0 relative">
          ...
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:text-slate-800 hover:bg-slate-100 transition-colors"
          >
            <X size={20} />
          </button>
        </div>
                ...
                >
                  ...
                  {topics.map((top) => (
                    <option key={top.id} value={top.id}>
                      Mod {top.course_modules?.title}: {top.title}
                    </option>
                  ))}
                </select>
              </div>
